...
{
	/** @var string Store the target browser */
	private $userBrowser='';
	/** @var string Store the target browser version */
	...
	/** @var string */
	private $output='';
	/** @var string Store the target group */
	private $userGroup='';
	/** @var string css buffer */
	private $css='';
	/** @var string */
	...
	private function setBrowserGroup($groups)
	{
		for ($i=0; $i<count($groups); $i++) {
			if ($groups[$i]['engine']==$this->userBrowser) {
				...
				elseif ($groups[$i]['greaterOrEqual']==0 && $groups[$i]['version']>=$this->userVersion) {
					$this->userGroup=$groups[$i]['grade'];
					break;
					}
				}
