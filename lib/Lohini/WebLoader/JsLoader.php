...
extends WebLoader
{
	...
	public function __construct(\Nette\ComponentModel\IContainer $parent=NULL, $name=NULL)
	{
		...
		$this->generatedFileNamePrefix='jsldr-';
		$this->generatedFileNameSuffix='.js';
		$this->sourcePath=WWW_DIR.'/js';
		$this->sourceUri=$this->baseUrl.'js/';
	...
	public function renderFiles()
	{
		...
		if (($cnt=count($this->files))>0) {
			if ($this->enableDirect && $cnt==1 && $this->files[0][1]==self::COMPACT) {
				...
				echo $this->useHeadJs
					? $this->getHeadJsElement($this->getPresenter()->link(':Lohini:WebLoader:', $this->generate($filenames, $content)))
					: $this->getElement($this->getPresenter()->link(':Lohini:WebLoader:', $this->generate($filenames, $content)));
				}
			}
		// raw code az nakonec
			...
			switch ($file[1]) {
				case self::COMPACT:
					echo $this->enableDirect
						...
						: $this->getElement($this->getPresenter()->link(':WebLoader:', $this->generate([$file[0]])));
					break;
				case self::MINIFY:
					// dean edwards packer neumi cz/sk znaky!!
					if (Strings::endsWith($file[0], '.min.js')) { // already minified ?
					...
					elseif (is_file("$this->sourcePath/".substr($file[0], 0, -3).'.min.js')) { // have minified ?
						echo $this->enableDirect
							...
							: $this->getElement($this->getPresenter()->link(':WebLoader:', $this->generate([substr($file[0], 0, -3).'.min.js'])));
						}
					elseif (in_array('JSMin', $dc) || class_exists('JSMin')) { // minify
						$content=\JSMin::minify($this->loadFile($file[0]));
						echo $this->getElement($this->getPresenter()->link(':WebLoader:', $this->generate([$file[0]], $content)));
						}
					else {
						if ($this->throwExceptions) {
							if ($this->getPresenter(FALSE)->context->params['productionMode'])
				...
				case self::PACK:
					...
					elseif (is_file($pfile="$this->sourcePath/".substr($file[0], 0, -3).'.pack.js')) { // have packed ?
						echo $this->enableDirect
							...
							: $this->getElement($this->getPresenter()->link(':WebLoader:', $this->generate([substr($file[0], 0, -3).'.pack.js'])));
						}
					elseif (in_array('JavaScriptPacker', $dc) || class_exists('JavaScriptPacker')) {
						$jsp=new \JavaScriptPacker($this->loadFile($file[0]));
						$content=$jsp->pack();
